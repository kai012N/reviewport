...
### Added
- **Per-change note input** in the panel — write how a change should be fixed; it's
  included in the exported fix-list (human-readable list + a machine-readable `notes`
  map) so the agent gets your exact instruction. The note is rendered XSS-safely (set as
  a value, never interpolated into HTML) and the export escapes `<`/`>`.
