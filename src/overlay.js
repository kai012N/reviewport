...
export function reviewportOverlay() {
  ...
  function navTo(c) {
    // SECURITY: the manifest is untrusted input. Only navigate to a SAME-ORIGIN
    // path — never let a route like "javascript:", "data:", or "//evil" reach
    // location.assign (that would be a DOM-XSS / off-origin redirect sink). The URL
    // parser normalizes leading whitespace and case, so scheme-smuggling is covered.
    var u;
    try { u = new URL(ROUTEBASE + c.route, location.origin); }
    catch (e) { notFound = true; paint(); return; }
    if (u.origin !== location.origin) { notFound = true; paint(); return; }
    location.assign(u.pathname + u.search + u.hash);
  }
  // Selector helpers that never throw on a malformed manifest selector (a bad
  ...
  function addHi(el, strong) {
    // Flashlight off = leave the page completely untouched (no fill, no outline), so
    // nothing covers the content being verified. It still scrolls to the change and the
    // panel still shows which one it is; use "Re-locate" or toggle the light back on.
    if (!fillOn) return;
  ...
  function markCode(marker, selector) {
    ...
    var scopes = selector ? safeQSA(document, selector) : [document];
    var best = null;
  ...
  function paint() {
    ...
    P.innerHTML =
      ...
      + '<button id="rv_fill" title="Toggle the on-page highlight — turn it off if it covers the content you are checking" style="box-sizing:border-box;flex:0 0 auto;display:inline-flex;align-items:center;justify-content:center;width:30px;height:24px;padding:0;border:1px solid #2c5f60;border-radius:5px;cursor:pointer;background:' + (fillOn ? '#0c5b57' : 'transparent') + ';color:' + (fillOn ? '#e8f4f3' : '#5a7e7d') + '">'
      ...
      + '<button id="rv_next" style="flex:1;padding:9px;border:1px solid #dfe7e6;background:#fff;border-radius:7px;cursor:pointer;font-size:14px;font-family:inherit">Next →</button>'
      + '<button id="rv_last" title="Last change" style="flex:0 0 auto;padding:9px 10px;border:1px solid #dfe7e6;background:#fff;border-radius:7px;cursor:pointer;font-size:13px;font-family:inherit">⏭</button></div>'
    ...
    P.querySelector('#rv_last').onclick = function () { go(CH.length - 1); };
  ...
  document.addEventListener('keydown', function (e) {
    ...
    else if (e.key === 'End') go(CH.length - 1);
